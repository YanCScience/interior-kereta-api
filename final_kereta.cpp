...
// Batas gerak kamera di area gerbong. Collision kursi menjaga kamera tidak
// masuk ke bangku, sehingga pemain masih bisa sedikit mendekati area kursi.
const float BATAS_X_MIN = -1.25f;
const float BATAS_X_MAX = 1.25f;
const float BATAS_Z_MIN = -9.0f;
const float BATAS_Z_MAX = 9.0f;
const float FOV_KAMERA = 68.0f;
...
void drawBox(float x0, float y0, float z0, float x1, float y1, float z1) {
    // Beberapa hasil integrasi memanggil drawBox dengan urutan koordinat
    // terbalik. Normalisasi min/max menjaga winding dan normal tetap stabil.
    float xmin = minf(x0, x1), xmax = maxf(x0, x1);
    float ymin = minf(y0, y1), ymax = maxf(y0, y1);
    float zmin = minf(z0, z1), zmax = maxf(z0, z1);

    glBegin(GL_QUADS);
    glNormal3f(0, -1, 0);
    glVertex3f(xmin, ymin, zmin); glVertex3f(xmax, ymin, zmin); glVertex3f(xmax, ymin, zmax); glVertex3f(xmin, ymin, zmax);
    glNormal3f(0, 1, 0);
    ...
    glNormal3f(0, 0, -1);
    glVertex3f(xmin, ymin, zmin); glVertex3f(xmin, ymax, zmin); glVertex3f(xmax, ymax, zmin); glVertex3f(xmax, ymin, zmin);
    glNormal3f(-1, 0, 0);
    glVertex3f(xmin, ymin, zmin); glVertex3f(xmin, ymin, zmax); glVertex3f(xmin, ymax, zmax); glVertex3f(xmin, ymax, zmin);
    glNormal3f(1, 0, 0);
    glVertex3f(xmax, ymin, zmin); glVertex3f(xmax, ymax, zmin); glVertex3f(xmax, ymax, zmax); glVertex3f(xmax, ymin, zmax);
    glEnd();
}
...
void drawGerbong() {
    ...
    setMaterial(0.45f, 0.28f, 0.14f, 0.08f, 0.06f, 0.04f, 12.0f);
    glColor3f(0.45f, 0.28f, 0.14f); drawBox(WALL_L, FLOOR_Y - TW * 2, FRONT_Z, WALL_R, FLOOR_Y, BACK_Z);
    // Panel lantai dibuat sedikit lebih tebal/tinggi agar tidak bertarung dengan lantai dasar.
    setMaterial(0.50f, 0.32f, 0.16f, 0.10f, 0.07f, 0.04f, 16.0f);
    glColor3f(0.50f, 0.32f, 0.16f); drawBox(WALL_L + 0.01f, FLOOR_Y + 0.010f, FRONT_Z, -0.50f, FLOOR_Y + 0.024f, BACK_Z);
    glColor3f(0.50f, 0.32f, 0.16f); drawBox(0.50f, FLOOR_Y + 0.010f, FRONT_Z, WALL_R - 0.01f, FLOOR_Y + 0.024f, BACK_Z);
    setMaterial(0.50f, 0.08f, 0.08f, 0.06f, 0.02f, 0.02f, 10.0f);
    glColor3f(0.50f, 0.08f, 0.08f); drawBox(-0.50f, FLOOR_Y + 0.018f, FRONT_Z, 0.50f, FLOOR_Y + 0.036f, BACK_Z);

    setMaterial(0.93f, 0.91f, 0.86f, 0.12f, 0.12f, 0.10f, 18.0f);
    ...
    setMaterial(0.72f, 0.69f, 0.63f, 0.10f, 0.10f, 0.08f, 14.0f);
    ...
    for (int sisi = 0; sisi < 2; sisi++) {
        ...
        setMaterial(0.76f, 0.73f, 0.66f, 0.12f, 0.11f, 0.09f, 14.0f);
        ...
        setMaterial(0.65f, 0.62f, 0.57f, 0.16f, 0.15f, 0.13f, 20.0f);
...
void setupPencahayaan() {
    if (lampuHidup) {
        ...
        // Ambient dibuat lebih rendah agar diffuse/specular memberi volume,
        // tetapi tetap cukup terang untuk demo kelas tanpa shader.
        GLfloat ambNormal[] = { 0.14f, 0.13f, 0.12f, 1.0f };
        glLightModelfv(GL_LIGHT_MODEL_AMBIENT, ambNormal);

        // LIGHT0: Plafon (Kuning Hangat)
        glEnable(GL_LIGHT0);
        GLfloat pos0[] = { 0.0f,  2.4f,  0.0f,  1.0f };
        GLfloat amb0[] = { 0.10f, 0.08f, 0.05f, 1.0f };
        GLfloat dif0[] = { 1.00f, 0.90f, 0.62f, 1.0f };
        GLfloat spec0[] = { 0.65f, 0.58f, 0.38f, 1.0f };
        ...
        glLightf(GL_LIGHT0, GL_QUADRATIC_ATTENUATION, 0.012f);

        // LIGHT1: Cahaya Jendela (Biru Langit)
        glEnable(GL_LIGHT1);
        GLfloat pos1[] = { -4.0f,  1.5f, -2.0f,  1.0f };
        GLfloat amb1[] = { 0.02f, 0.04f, 0.07f, 1.0f };
        GLfloat dif1[] = { 0.26f, 0.46f, 0.85f, 1.0f };
        GLfloat spec1[] = { 0.06f, 0.12f, 0.22f, 1.0f };
        ...
        GLfloat amb2[] = { 0.04f, 0.01f, 0.01f, 1.0f };
        GLfloat dif2[] = { 0.12f, 0.03f, 0.03f, 1.0f };
        ...
        glEnable(GL_LIGHTING);
        GLfloat ambGelap[] = { 0.08f, 0.07f, 0.06f, 1.0f };
...
void drawHUD() {
    ...
    bool wasDepth = (glIsEnabled(GL_DEPTH_TEST) == GL_TRUE);
    bool wasBlend = (glIsEnabled(GL_BLEND) == GL_TRUE);

    glDisable(GL_LIGHTING);
    glDisable(GL_DEPTH_TEST);   // Bitmap text 2D harus bebas dari isi depth buffer 3D.
    ...
    glBegin(GL_QUADS); glVertex2f(0, winH); glVertex2f(455, winH); glVertex2f(455, winH - 138); glVertex2f(0, winH - 138); glEnd();
    ...
    drawText2D(10, (float)y, "J/L: Putar kamera  |  I/K: Lihat atas-bawah"); y -= 16;
    drawText2D(10, (float)y, "Mouse Drag: Lihat sekeliling  |  F: Toggle lampu"); y -= 16;
    drawText2D(10, (float)y, "ESC: Keluar program"); y -= 18;

    char buf[128];
    sprintf(buf, "Lampu: %s  |  Pos: (%.2f, %.2f)  |  Yaw/Pitch: %.0f / %.0f",
        lampuHidup ? "ON " : "OFF", camX, camZ, yaw, pitch);
...
void applyCameraView() {
    ...
    float shakeX = 0.006f * sin(shakeT * 13.0f);
    float shakeY = getaran;
    gluLookAt(camX + shakeX, camY + shakeY, camZ, lookX + shakeX, lookY + shakeY * 0.35f, lookZ, 0.0f, 1.0f, 0.0f);
}

bool isInsideKursi(float x, float z) {
    // Collision sederhana berbasis AABB. Nilai dibuat sedikit lebih besar dari
    // model kursi agar kamera terasa seperti tubuh, bukan titik tanpa volume.
    float posX[] = { -1.05f, -0.65f, +0.65f, +1.05f };
    const float startZ = -8.50f, jarakZ = 1.70f;
    const int jumlahBaris = 10;
    const int jumlahKolom = 4;
    const float HALF_W = 0.30f;
    const float HALF_D = 0.38f;

    for (int baris = 0; baris < jumlahBaris; baris++) {
        float pz = startZ + (float)baris * jarakZ;
        for (int kolom = 0; kolom < jumlahKolom; kolom++) {
            if (x >= posX[kolom] - HALF_W && x <= posX[kolom] + HALF_W &&
                z >= pz - HALF_D && z <= pz + HALF_D) {
                return true;
            }
        }
    }
...
void terapkanBatasDanCollision(float kandidatX, float kandidatZ) {
    // Uji X dan Z terpisah agar kamera bisa sliding di pinggir kursi.
    if (kandidatX >= BATAS_X_MIN && kandidatX <= BATAS_X_MAX && !isInsideKursi(kandidatX, camZ)) {
        camX = kandidatX;
    }
    if (kandidatZ >= BATAS_Z_MIN && kandidatZ <= BATAS_Z_MAX && !isInsideKursi(camX, kandidatZ)) {
        camZ = kandidatZ;
    }
}
...
void keyboardDown(unsigned char key, int x, int y) {
    ...
    if (pitch < -80.0f) pitch = -80.0f;
    terapkanBatasDanCollision(newX, newZ);
...
void reshape(int w, int h) {
    ...
    gluPerspective(FOV_KAMERA, (double)w / h, 0.05, 500.0);
    glMatrixMode(GL_MODELVIEW);
}
...
int main(int argc, char* argv[]) {
    ...
    printf("  OpenGL fixed-function + freeGLUT\n");
    printf("==================================================\n");
    printf("  W/S  : Maju / mundur\n");
    printf("  A/D  : Geser kiri / kanan\n");
    printf("  J/L  : Putar kamera kiri / kanan\n");
    printf("  I/K  : Lihat atas / bawah\n");
    printf("  Mouse drag : Putar kamera bebas\n");
    printf("  F    : Toggle lampu interior\n");
    printf("  ESC  : Keluar\n");
    printf("==================================================\n");
